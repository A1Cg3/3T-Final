...
mysql -u root -p1234 -e "drop database if exists 3T;"
...
const router = Router();router.get('/', (req, res) => res.send('LA API ES FUNCIONAL '))

router.post('/clientes',     controllers.createCliente)
router.get( '/allclientes',     controllers.getAllClientes)
...
app.use('/api', routes);
app.listen(PORT, () => console.log('Esta ejecutandose en el puerto' , PORT))
SERVER.JS
